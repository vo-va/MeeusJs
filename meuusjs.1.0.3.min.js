...
A.Rise = {
	...
	times: function(eclcoord, deltaT, h0, Th0, eqcoord3) {
		...
		// see adapteqcoord3 for explanation, in 2 words, to correctly interplated we need to use right ascension with the same sign

		var adataptedRa = A.Rise.adapteqcoord3(eqcoord3);
		console.log(adataptedRa)

		
		var d3ra = A.Interp.newLen3(-86400, 86400, adataptedRa[0], adataptedRa[1], adataptedRa[2]);
		var d3dec = A.Interp.newLen3(-86400, 86400, [eqcoord3[0].dec, eqcoord3[1].dec, eqcoord3[2].dec]);

		// if ra after interpoation more that 180 degrees 2 Math.PI
		// or less than -180 degrees -2 Math.PI convert to fit into 180 degrees range with + or - sign
	...
	/**
	 * Because Interpolation based on funcion that is monotonic, then if we will have mix of Right Ascension with positive
	 * and negative values, interpoalaion will be broken. The sign of RA might change around equinoxes, ie it can become more than 180 degrees
	 * or less. So function will analize if all RA coordination have the same sing, if not it will conver coordinate to positin or negative radians.
	 *
	 * After interpolation is done result needs to be adapted back, i.e. if coordiates more than 180 or Math.pi (3.14) do the convertion to fit into 180 degrees range iwth + or - sign.
	 *
	 * @param {A.EclCoord} eclcoord - ecliptic coordinates of observer on Earth
	*/
	adapteqcoord3: function(eqcoord3) {
		// if all values have the same sign do nothing
		if (
			(eqcoord3[0].ra >= 0 && eqcoord3[1].ra >= 0 && eqcoord3[2].ra >= 0)
			|| (eqcoord3[0].ra < 0 && eqcoord3[1].ra < 0 && eqcoord3[2].ra < 0)
		)

		{
			return [eqcoord3[0].ra, eqcoord3[1].ra, eqcoord3[2].ra];
		}

		// count what sign is prevailing
		/** @type {number} */
		var positive = 0;
		...
		if (positive > negative) {
			return map(Astro.Rise.toPositiveRadians, [eqcoord3[0].ra, eqcoord3[1].ra, eqcoord3[2].ra]);
		} else {
			return map(Astro.Rise.toNegativeRadians, [eqcoord3[0].ra, eqcoord3[1].ra, eqcoord3[2].ra]);
		}
	},

	/**
	 * Will convert negative radians to positive radians
	 * @param {number} radians - negative value in radians (ie. more than -180 degrees)
	 * @return {number} radians, converted to positive
	 *
	 */
	toPositiveRadians: function(radians) {
		if (radians < 0) {
	...
	/**
	 * Will convert positive radians to negative radians
	 * @param {number} radians - negative value in radians (ie. more than +180 degrees)
	 * @return {number} radians, converted to positive
	 */
	toNegativeRadians: function(radians) {
		if (radians > Math.PI) {
			return radians - 2 * Math.PI;
		}
		return radians;
	...
	/** Convert radians value that more that 180 degres or less that -180 to the negative or posive value with reversed sign
	 * @param {number} radians - radians value
	 */
	limitTo180: function(radians) {
		if (radians > Math.PI) {
			return radians - 2 * Math.PI;
		}
